  ...
  class Base
    ...
    def self.pg_copy_to path = nil, options = {}
      ...
      if path
        ...
        connection.execute "COPY (#{options[:sql] || self.all.to_sql}) TO #{sanitize(path)} WITH #{options_string}"
      else
        connection.execute "COPY (#{options[:sql] || self.all.to_sql}) TO STDOUT WITH #{options_string}"
